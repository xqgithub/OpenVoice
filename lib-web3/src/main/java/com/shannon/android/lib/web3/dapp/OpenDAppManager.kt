 ...
 *
 * @Package:        com.shannon.android.lib.web3.dapp
 * @ClassName:      OpenDAppManager
 * @Description:     作用描述
 * @Author:         czhen
 * @CreateDate:     2022/11/10 17:39
 */
class OpenDAppManager(
    socket: Socket,
    ...
    override fun sendSessionRequest(chainId: Int?) {
        ...
        coroutineScope.launch {
            ...
            // notify dApp
            callback(
                SessionCallback.SessionRequested(
                messageId,
                chainId,
    ...
    override suspend fun sendRequest(method: JsonRpcMethod,
        ...
        if (!sessionApproved.get()) {
            ...
            failureCallback(
                Failure(type = FailureType.SessionError,
        ...
        if (finalMessageId != null) {
            ...
            try {
                when (method) {
                    is CustomRpcMethod -> {
                        callback(
                            RequestCallback.CustomRequested(messageId,
                            method.value,
                            data))
                    }
                    ...
                    is EthRpcMethod -> {
                        when (method) {
                            EthRpcMethod.Sign -> {
                                callback(
                                    RequestCallback.EthSignRequested(messageId,
                            ...
                            EthRpcMethod.SignTypedData -> {
                                callback(
                                    RequestCallback.EthSignTypedDataRequested(messageId,
                                    data[0] as String,
                                    data[1]))
                            }
                            ...
                            EthRpcMethod.SignTransaction -> {
                                callback(
                                    RequestCallback.EthSignTxRequested(messageId,
                                    data[0] as EthTransaction
                                    ))
                            }
                            EthRpcMethod.SendRawTransaction -> {
                                callback(
                                    RequestCallback.EthSendRawTxRequested(messageId,
                                    data[0] as String))
                            }
                            EthRpcMethod.SendTransaction -> {
                                callback(
                                    RequestCallback.EthSendTxRequested(messageId,
                                    data[0] as EthTransaction
                                    ))
                            }
                        }
                        ...
                        failureCallback(
                            Failure(type = FailureType.InvalidRequest,
            ...
            when (method) {
                null -> {
                    ...
                    reportInvalidSocketMessage(messageId,
                        JsonRpcErrorData(
                            RpcErrorCode.RequestMethodNotAvailable,
    ...
    override suspend fun handleJsonRpcResponse(decryptedPayload: String,
        ...
        if (messageId == sessionRequestId || sessionResponsePayload != null) {
            ...
            messageIdCallbackMap.remove(messageId)
            callback(
                SessionCallback.SessionApproved(
                messageId = messageId,
                chainId = sessionResponsePayload.result.chainId,
                accounts = sessionResponsePayload.result.accounts,
                remotePeerId = sessionResponsePayload.result.peerId,
        ...
        when (val method = messageMethodMap[messageId]) {
            is CustomRpcMethod -> {
                ...
                // notify dApp
                messageIdCallbackMap[messageId]?.invoke(
                    RequestCallback.CustomResponse(messageId,
                    payload.result))
                messageIdCallbackMap.remove(messageId)
                callback(
                    RequestCallback.CustomResponse(messageId,
            ...
            is EthRpcMethod -> {
                when (method) {
                    EthRpcMethod.Sign -> {
                        ...
                        // notify dApp
                        messageIdCallbackMap[messageId]?.invoke(
                            RequestCallback.EthSignResponse(messageId,
                            payload.result))
                        messageIdCallbackMap.remove(messageId)
                        callback(
                            RequestCallback.EthSignResponse(messageId,
                    ...
                    EthRpcMethod.SignTransaction -> {
                        ...
                        // notify dApp
                        messageIdCallbackMap[messageId]?.invoke(
                            RequestCallback.EthSignTxResponse(messageId,
                            payload.result))
                        messageIdCallbackMap.remove(messageId)
                        callback(
                            RequestCallback.EthSignTxResponse(messageId,
                    ...
                    EthRpcMethod.SendRawTransaction -> {
                        ...
                        // notify dApp
                        messageIdCallbackMap[messageId]?.invoke(
                            RequestCallback.EthSendRawTxResponse(messageId,
                    ...
                    EthRpcMethod.SendTransaction -> {
                        ...
                        // notify dApp
                        messageIdCallbackMap[messageId]?.invoke(
                            RequestCallback.EthSendTxResponse(messageId,
                            payload.result as? String?))
                        messageIdCallbackMap.remove(messageId)
                        callback(
                            RequestCallback.EthSendTxResponse(messageId,
                ...
                messageIdCallbackMap.remove(messageId)
                failureCallback(
                    Failure(type = FailureType.InvalidResponse,
    ...
    override suspend fun handleJsonRpcError(decryptedPayload: String,
        ...
        if (sessionApproved.get()) {
            ...
            if (messageMethodMap[messageId] != null) {
                // notify dApp
                messageIdCallbackMap[messageId]?.invoke(
                    RequestCallback.RequestRejected(messageId,
                    payload.error))
                messageIdCallbackMap.remove(messageId)
                callback(
                    RequestCallback.RequestRejected(messageId,
    ...
    fun socketInitialized() = initialized.get()

    fun sessionApproved() = sessionApproved.get()

    suspend fun closeRemoteSocket(state: SessionState) {
        val model = SessionUpdate(
            approved = false,
            chainId = null,
            accounts = null
        )
        ...
        val payloadAsJson = serializePayload(payload, getRequestType( SessionUpdate::class.java)) ?:return

        val encryptedPayload = encryptPayload(payloadAsJson) ?:return
        ...
        val remotePeerId = state.remotePeerId
