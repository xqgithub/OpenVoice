...
class SearchTopicsAdapter(
    ...
    BaseFunBindingRecyclerViewAdapter<HashTag, AdapterSearchTopicsBinding>(), AdapterLogicListener {
    ...
    override fun bindView(
        holder: BaseByViewHolder<HashTag>,
        binding: AdapterSearchTopicsBinding,
        bean: HashTag,
        position: Int
    ) {
        binding.apply {
            tvName.text = "${bean.name}"
            tvTopicPublishedNumber.text =
                "${addComma("${bean.statusesCount}")} ${mContext.getString(R.string.openvoiceover)}"
    ...
    override fun bindView(binding: AdapterSearchTopicsBinding, bean: HashTag, position: Int) {
    }
     ...
     * 设置喜欢按钮
     */
    private fun setFavourited(favourited: Boolean, binding: AdapterSearchTopicsBinding) {
