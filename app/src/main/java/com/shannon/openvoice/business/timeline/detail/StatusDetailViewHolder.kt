...
import com.shannon.openvoice.business.timeline.MediaAdapter
 ...
 *
 * @Package:        com.shannon.openvoice.business.timeline.detail
 * @ClassName:      StatusDetailViewHolder
 * @Description:     java类作用描述
 * @Author:         czhen
 * @CreateDate:     2022/8/2 11:37
 */
class StatusDetailViewHolder(
    val binding: ItemStatusDetailBinding,
    private val actionListener: StatusActionListener
) :
    BaseFunBindingViewHolder<StatusModel, ItemStatusDetailBinding>(
        binding
    ) {
    ...
    override fun onBindView(binding: ItemStatusDetailBinding, bean: StatusModel, position: Int) {
    ...
    private fun holderBindView(bean: StatusModel, position: Int) {
        ...
        binding.run {
            ...
            setNameInfo(bean.account.name, bean.account.username)
            setTimestampInfo(actionableStatus.createdAt)
            setPlayStatus(bean.isPlaying, actionableStatus.ttsGenerated)
    ...
    private fun setNameInfo(displayName: String, userName: String) {
        binding.statusDisplayName.text = displayName
        binding.statusUsername.text = mContext.getString(R.string.post_username_format, userName)
    }
    ...
    private fun setPlayStatus(isPlaying: Boolean, ttsGenerated: Boolean) {
        Timber.e("22 playingState = $holderPosition ; $isPlaying")
    ...
    private fun setReblogEnabled(enabled: Boolean, visibility: StatusModel.Visibility) {
        ...
        if (enabled) {
            if (visibility == StatusModel.Visibility.PRIVATE) {
                binding.statusInset.setInactiveImage(R.drawable.icon_reblog_private_normal)
                binding.statusInset.setActiveImage(R.drawable.icon_reblog_private_active)
            } else {
            ...
            binding.statusInset.setInactiveImage(R.drawable.icon_reblog_private_normal)
            binding.statusInset.setActiveImage(R.drawable.icon_reblog_private_normal)
        }
    }
