...
import static com.shannon.openvoice.FunApplication.appViewModel;
...
public class NotificationHelperUtils implements NotificationBrodcaseReceiver.ICallBack {
    ...
    //通道渠道id
    public static final String PRIMARY_CHANNEL = "fcm_default_channel";
    //通道渠道id
    ...
    public void openPermission(Activity activity) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            appViewModel.toPermissionSetting(activity);
        }
    }
...
//        builder.setSmallIcon(TKExtManage.getInstance().getAppLogoRes(context));//小图标
        builder.setSmallIcon(R.drawable.icon_launcher);//小图标
        builder.setLargeIcon(BitmapFactory.decodeResource(context.getResources(), R.drawable.icon_launcher));//大图标，未设置时使用小图标代替
//
...
//        notification.flags = Notification.FLAG_ONLY_ALERT_ONCE | Notification.FLAG_AUTO_CANCEL;
    ...
    public void sendNotification2(Context context, Class<?> cla, int id,
...
//        builder.setChronometerCountDown(false);//设置时钟是否为倒计时
         ...
         * PendingIntent.FLAG_UPDATE_CURRENT 如果要创建的PendingIntent已经存在了，那么在保留原先PendingIntent的同时，将原先PendingIntent封装的Intent中的extra部分替换为现在新创建的PendingIntent的intent中extra的内容
         */
        PendingIntent pendingIntent = PendingIntent.getActivity(context, pushid, intent, PendingIntent.FLAG_IMMUTABLE);
        builder.setContentIntent(pendingIntent);//提供单击通知时发送的PendingIntent
...
//        //关闭按钮点击事件
//        Intent intent = new Intent();
//        intent.setAction(ConfigConstants.CONSTANT.NOTIFACATIO_CLOSE);
